...
var PokemonModel = Backbone.Model.extend({
  ...
  initialize: function(opts) {
    ...
    if (!this.get('name')) {
      throw new Error('Must specify name or id when creating a Pokemon');
    }
  ...
  getPokemon: function() {
    ...
    var pkmn = Object.keys(Pokedex.data).map(function(x) {
      ...
      if (self.get('name')) {
        ...
        throw new Error('No name or id passed or set to get pokemon information');
      }
    })[0];
  ...
  getAvailableMoveNames: function() {
    var self = this;
    var pkmnKey = Object.keys(Pokedex.data).filter(function(x) {
      return Pokedex.data[x].species === self.get('name');
    })[0]; // TODO deal with forms

    var moveKeys = Object.keys(Learnset.data[pkmnKey].learnset);
    return moveKeys.map(function(x) {
